...
public class DbMpaRatingStorage implements MpaRatingStorage{
    ...
    public List<MpaRating> getMpaRatingList() {
        return mpaRating.query(UtilReader.readString(SELECT_ALL_SQL_PATH), new MpaRatingMapper());
    }

    @Override
    public MpaRating getMpaRating(Integer ratingId) {
        return mpaRating.query(UtilReader.readString(SELECT_BY_ID_SQL_PATH), new MpaRatingMapper(), ratingId)
