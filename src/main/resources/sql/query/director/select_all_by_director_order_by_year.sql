SELECT FL.FILM_ID AS film_id,              -- идентификатор фильма
       ...
       DR.DIRECTOR_ID AS DIRECTOR_ID,      -- идентификатор режиссера
       DR.DIRECTOR_NAME AS DIRECTOR_NAME,  -- имя режиссера
       (SELECT COUNT(*)
        FROM FILM_LIKES
...
FROM FILMS AS FL                           -- таблица с фильмами
         ...
         LEFT JOIN FILM_DIRECTORS AS FI_DR ON FL.FILM_ID = FI_DR.FILM_ID -- связь с таблиц films >-< director
         LEFT JOIN DIRECTORS AS DR ON FI_DR.DIRECTOR_ID = DR.DIRECTOR_ID -- связь с таблицей режиссеров
...
GROUP BY FL.FILM_ID, DR.DIRECTOR_ID, GENRE_NAME
ORDER BY FL.RELEASE_DATE, rate DESC, FL.FILM_ID, GENRE_NAME, DIRECTOR_NAME                 -- количество выгружаемых записей
